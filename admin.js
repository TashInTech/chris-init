...
function renderCatalog(products) {
  ...
  wrap.innerHTML = header + products.map((p) => `
  ...
  wrap.querySelectorAll('.btn-edit').forEach((btn) =>
    btn.addEventListener('click', () =>
      openProductModal(products.find((p) => p.id === Number(btn.dataset.id)))
    )
  );
    ...
    <div class="modal-overlay">
      <div class="modal-card">
        ...
        <div class="modal-field">
          <label>Name</label>
          <input id="pName" value="${isEdit ? escapeHtmlA(product.name) : ''}" />
        </div>
        <div class="modal-field">
          ...
          ${isEdit
            ? `<img src="${product.img}" alt="" style="width:60px;height:60px;object-fit:cover;border-radius:8px;margin-top:8px;" id="pImgPreview" />`
            : `<img id="pImgPreview" style="display:none;width:60px;height:60px;object-fit:cover;border-radius:8px;margin-top:8px;" />`
          }
          <div class="modal-error" id="pImgError" style="margin-top:4px;"></div>
        </div>
        <div class="modal-field">
          <label>Price (UGX)</label>
          <input id="pPrice" type="number" value="${isEdit ? product.price : ''}" />
        </div>
        <div class="modal-field">
          <label>Rating (1-5)</label>
          <input id="pRating" type="number" min="1" max="5" value="${isEdit ? product.rating : 5}" />
        </div>
        <div class="modal-field">
          <label>Review Count</label>
          <input id="pCount" type="number" value="${isEdit ? product.count : 0}" />
        </div>
        <div class="modal-field">
          <label>Badge (optional)</label>
          <input id="pBadge" value="${isEdit ? escapeHtmlA(product.badge) : ''}" placeholder="New, Hot, Best…" />
        </div>
...
// ---------------------------------------------------
// ORDERS PIPELINE — mobile-first stacked cards
// ---------------------------------------------------
...
function renderOrders(orders) {
  ...
  if (!orders || orders.length === 0) {
    wrap.innerHTML = `<p style="padding:20px;color:var(--text-mid);">No orders yet.</p>`;
    return;
  }
  ...
  wrap.innerHTML = orders.map((o) => {
    const itemsSummary = Array.isArray(o.items)
      ...
      : '';
    const total = `UGX ${Number(o.total_price).toLocaleString('en-UG')}`;
    const statusClass = `status-${o.payment_status}`;

    return `
      <div class="table-row order-row">
        <div class="order-field">
          <span class="order-label">Customer</span>
          <span class="order-value">${escapeHtmlA(o.customer_name)}</span>
        </div>
        <div class="order-field">
          <span class="order-label">Email</span>
          <span class="order-value">${escapeHtmlA(o.customer_email)}</span>
        </div>
        <div class="order-field">
          <span class="order-label">Phone</span>
          <span class="order-value">${escapeHtmlA(o.customer_phone)}</span>
        </div>
        <div class="order-field">
          <span class="order-label">Items</span>
          <span class="order-value">${itemsSummary}</span>
        </div>
        <div class="order-field">
          <span class="order-label">Total</span>
          <span class="order-value" style="color:var(--orange);font-weight:700;">${total}</span>
        </div>
        <div class="order-field">
          <span class="order-label">Status</span>
          <span class="order-value">
            <span class="status-pill ${statusClass}">${o.payment_status}</span>
          </span>
        </div>
      </div>`;
  }).join('');
