...
let allSupabaseProducts = []; // store all for load-more
let supabaseMoreLoaded = false;
...
async function loadSupabaseProducts() {
  ...
  try {
    ...
    if (products && products.length > 0) {
      allSupabaseProducts = products;
      grid.innerHTML = '';

      // Show only first 6 on page load
      const initial = products.slice(0, 6);
      initial.forEach(p => grid.insertAdjacentHTML('beforeend', buildProductCard(p)));

      // Show or hide the More button depending on whether there are more
      if (moreBtn) {
        if (products.length > 6) {
          moreBtn.style.display = 'inline-flex';
          moreBtn.onclick = loadMoreSupabaseProducts;
        } else {
          moreBtn.style.display = 'none';
        }
      }

      // Live updates: re-render respecting load-more state
      subscribeToProducts(async () => {
        const fresh = await fetchProducts();
        allSupabaseProducts = fresh;
        grid.innerHTML = '';
        const toShow = supabaseMoreLoaded ? fresh : fresh.slice(0, 6);
        toShow.forEach(p => grid.insertAdjacentHTML('beforeend', buildProductCard(p)));
      });
...
function loadMoreSupabaseProducts() {
  if (supabaseMoreLoaded) return;
  supabaseMoreLoaded = true;
  const grid = document.getElementById('productGrid');
  const moreBtn = document.getElementById('moreBtn');

  // Add remaining products with fade-in animation
  const remaining = allSupabaseProducts.slice(6);
  remaining.forEach(p => {
  ...
  if (moreBtn) {
    moreBtn.textContent = '✓ Full Collection Loaded';
